...
def sim2Dspec3(t21, laser_lam, laser_fwhm, mu1, mu2, Gam, sigI, monoC_lam, epsilon0, omega0, lam): # optimize these if using CD results doesn't work
    ...
    #     ax_lim = [13.6, 15.75]
    #     plot2Dspectra(ax1, ax2, FT_dqc_temp, n_cont=15, ax_lim=ax_lim, timing_mode=timing_mode,title=r'DQC($\omega$) with '+timing_mode_str,domain='freq')#'($\tau_{32}$ = 0)',domain = 'freq')
    #     # plot2Dspectra(ax1, ax2, FT_nrp_temp, n_cont, ax_lim=ax_lim, timing_mode=timing_mode,title=r'NRP($\omega$) with '+timing_mode_str,domain='freq')#'($\tau_{32}$ = 0)',domain = 'freq')
    #     # plot2Dspectra(ax1, ax2, FT_rp_temp,n_cont, ax_lim=ax_lim, timing_mode=timing_mode,title=r'RP($\omega$) with '+timing_mode_str,domain='freq')#'($\tau_{32}$ = 0)',domain = 'freq')
    #     # plt.show()
