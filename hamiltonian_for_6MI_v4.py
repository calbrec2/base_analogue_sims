...
#  need these for the omega21, omega_32, omega_43 values
# =============================================================================
...
def monomerCD_opt(params,plot_mode = 0):
    lam, epsilon0, omega0 = params
    omega_ge=epsilon0/2 # virtual state energy spacing
    ...
    # matrix_plotter(hamA, alpha, alpha, title=r'Hamiltonian of monomer $(cm^{-1} x10^{3})$',size=nEle*nVib,title_fontsize=20,label_fontsize=16,fontsize=22)#14)
    # matrix_plotter(h6A, alpha, alpha, title=r'Hamiltonian of monomer $(cm^{-1} x10^{3})$',size=nEle*nVib,title_fontsize=20,label_fontsize=16,fontsize=22)
    h1A = epsilon0 * kr(cDc_uv, Ivib) # electronic levels
    h4A = omega0 * kr(Iel, bDb)  # vibrational levels
    h6A = omega0 * kr(cDc_uv, lam * (bD + b) + (lam**2)*Ivib) # coupling between electronic and vibrational
    hamA_uv = h1A + h4A + h6A
    # matrix_plotter(hamA_uv, alpha, alpha, title=r'Hamiltonian of monomer $(cm^{-1} x10^{3})$',size=nEle*nVib,title_fontsize=20,label_fontsize=16,fontsize=22)#14)
    
    
    # Diagonalize Hamiltonian
    # epsA, vecsA = la.eig(hamA)
    epsA, vecsA = la.eig(hamA_uv)
    ...
    # ===================================================================================================================
    # 20240206: ditching the idea of actually calculaing a monomer CD. Going to optimize an absorption calculation 
    # to the CD data to estimate lambda and omega0... then proceed with a dimer model that ignores the fact that 
    # the monomer has CD... allows us to avoid needing HT terms and monomer magnetic dipole moments for now
    # maybe someday a theorist will do the atomistic QM calculations and we can have a more precise model
    # see v3 of this code for my VERY INCORRECT attempt at the HT terms
    # ===================================================================================================================
    ...
    chisq = np.sum((simAbs[:,1] - cCDSpectrum[:,1])**2 )#/ cCDSpectrum[:,1])

    return chisq


# these work... 
lam = 3.0 #3 #1.5 #2 #0.71#1.54 # huang-rhys parameter: displacement of excited state well w.r.t. ground state well
epsilon0 = 28600 #29300 #29500 # energy gap from ground state (g) to real excited state (f)
omega0=80#100#80 #160#100 #800/2 # spacing of vibrational levels (set via ground state?)

# these are a bit better...
lam = 2.6 #3 #1.5 #2 #0.71#1.54 # huang-rhys parameter: displacement of excited state well w.r.t. ground state well
epsilon0 = 28500 #29300 #29500 # energy gap from ground state (g) to real excited state (f)
omega0=250#100#80 #160#100 #800/2 # spacing of vibrational levels (set via ground state?)
# .. need to optimize
params = [lam, epsilon0, omega0]
monomerCD_opt(params,plot_mode = 1)
...
omega0_bounds = [150, 300]
bounds = np.vstack([lam_bounds, epsilon0_bounds, omega0_bounds])
x0 = np.array([lam, epsilon0, omega0])
# res = minimize(monomerCD_opt, x0, method='nelder-mead',
#                options={'xatol': 1e-8, 'disp': True})
if __name__ == '__main__':
    res = opt.differential_evolution(func=monomerCD_opt, 
                                      bounds=bounds,
                                      x0=x0,
                                      disp=True,
                                      workers=1,
